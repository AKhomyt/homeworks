...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
Student.prototype.ajax = function(){
    ...
    xhr.send(JSON.stringify({id: 1, first_name: 'Zzzzzz', course: 2,estimate: 5, is_active: true}));
    ...
    xhr.onreadystatechange = function(){
        ...
        if (xhr.status == 200){
            console.log(JSON.parse(xhr.responseText).student);
        }
    }
