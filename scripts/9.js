...
Student.prototype.restart = function () {
    ...
    //    let restart = new Student(this.students, this.listContainer, this.statisticsContainer, this.url);
...
Student.prototype.getDATA = function(){
    ...
    xhr.onreadystatechange = function(){
        ...
        if (xhr.status == 200){
            restart.students = JSON.parse(xhr.response).students;
            restart.listOfStudents();
            restart.statistics();

            for (let i of restart.students){
                restart.ajaxDelete(i.id);
            }

        } else {
            restart.students = JSON.parse(localStorage.getItem('students'));
            restart.listOfStudents();
            restart.statistics();
        }
    }
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
Student.prototype.ajaxDelete = function(id_student){
    let xhr = new XMLHttpRequest();
    xhr.open('DELETE', this.url + '/' + id_student + '/', true);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
Student.prototype.ajaxPushStudent = function(student){
    let xhr = new XMLHttpRequest();
    ...
    xhr.send(JSON.stringify(student));//JSON.stringify(student)
    ...
    xhr.onreadystatechange = function(){
        ...
        if (xhr.status == 200){
            student.id = JSON.parse(xhr.response).student.id;
        } else {
            student.id = '';
        }
    }
...
//Данные студента и события формы ввода-------------------------------------------------------------------
        ...
        active.onchange = () => {
            this.students[i].is_active = active.checked;
            this.restart();
        }
        // Events<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    }
            ...
            if (regEmail.test(student.email)) {
                if (regCourse.test(student.course)) {
                    if (regEstimate.test(student.estimate)) {
                        student.id = mainObject.ajaxPushStudent(student);
                        mainObject.students.push(student);
                        mainObject.ajaxPushStudent(student);
                        mainObject.restart();
                    } else {
...
//     xhr.open('GET', '../scripts/phones.json', false);
//
// // 3. Отсылаем запрос
//     xhr.send();
//
...
//usage:
    readTextFile("../scripts/phones.json", function(text){
