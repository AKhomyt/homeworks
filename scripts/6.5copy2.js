...
Student.prototype.enterForm = function(textElement, size){
    //Локальный инпут для списка.
    textElement.onclick = function ev() {
        let event = this.onclick;
        this.onclick = null;
...
Student.prototype.statistics = function () {
    ...
    //this.statisticsContainer.innerHTML += "Всего неактивных студентов:" + noActiveAll;
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
//Данные студента(плюс события формы ввода)----------------------------------------------------------------
        ...
        //Инициализация локальных форм ввода.
